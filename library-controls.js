...
function restorePreferences(elements) {
  ...
  if (
    savedSort
    && VALID_SORT_ORDERS.has(savedSort)
...
function syncFavoriteChip(favoriteField, favoritesOnly) {
  ...
  favoriteField.setAttribute(
    "aria-label",
    active ? "즐겨찾기만 필터 해제" : "즐겨찾기만 필터 적용",
  );
...
function installLayout(elements) {
  ...
  if (listOptions && listOptions.childElementCount === 0) {
...
function bindPreferences(elements) {
  ...
  searchInput.addEventListener("input", () => {
    writePreference(STORAGE_KEYS.search, searchInput.value);
  });

  sortOrder.addEventListener("input", () => {
    if (VALID_SORT_ORDERS.has(sortOrder.value)) {
      writePreference(STORAGE_KEYS.sort, sortOrder.value);
    }
  });
