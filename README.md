...
사용자가 저장한 프롬프트는 서버나 GitHub 저장소로 전송되지 않습니다. 현재 브라우저의 IndexedDB 데이터베이스 `prompt-vault` 안에 저장됩니다.

브라우저 사이트 데이터 삭제, 브라우저 초기화 또는 기기 초기화 시 데이터가 삭제될 수 있으므로 JSON 백업을 정기적으로 보관해야 합니다.
