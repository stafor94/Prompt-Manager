...
test("동적으로 스타일을 불러오는 모듈도 1.9.0을 사용한다", async () => {
  ...
  for (const source of [library, sixColumns, viewer]) {
    assert.match(source, /APP_VERSION = "1\.9\.0"/);
    assert.doesNotMatch(source, /APP_VERSION = "1\.5\.3"/);
  }
});

test("1.9.0 변경 이력과 점진 로딩 코어가 앱 셸에 포함된다", async () => {
  const [changelog, sw] = await Promise.all([read("CHANGELOG.md"), read("sw.js")]);
  assert.match(changelog, /## \[1\.9\.0\] - 2026-09-10/);
