...
test("화면과 런타임 자산 버전이 v1.2.0으로 일치한다", async () => {
  ...
  assert.doesNotMatch(index, /\?v=1\.1\.3/);
...
test("서비스 워커가 HTTP 캐시를 우회해 신규 앱 셸을 확인한다", async () => {
  ...
  assert.match(serviceWorker, /prompt-manager-shell-v37/);
  assert.match(serviceWorker, /cache: "no-store"/);
  assert.match(serviceWorker, /url\.searchParams\.set\("pm-shell", CACHE_NAME\)/);
  assert.match(serviceWorker, /update-manager\.js\?v=1\.2\.0/);
