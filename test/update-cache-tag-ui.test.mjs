...
test("화면과 런타임 자산 버전이 v1.10.1으로 일치한다", async () => {
...
test("편집 화면에서 즐겨찾기 UI를 숨기고 카드 즐겨찾기 모듈을 로드한다", async () => {
  ...
  assert.match(index, /card-favorite\.js\?v=1\.10\.1/);
  assert.match(index, /favorite-editor-ui\.css\?v=1\.10\.1/);
...
test("서비스 워커가 HTTP 캐시를 우회해 신규 앱 셸을 확인한다", async () => {
  ...
  assert.match(serviceWorker, /prompt-manager-shell-v56/);
  assert.match(serviceWorker, /cache: "no-store"/);
  assert.match(serviceWorker, /url\.searchParams\.set\("pm-shell", CACHE_NAME\)/);
  assert.match(serviceWorker, /update-manager\.js\?v=1\.10\.1/);
  assert.match(serviceWorker, /editor-title-extractor\.mjs/);
  assert.match(serviceWorker, /card-favorite\.js\?v=1\.10\.1/);
  ...
  assert.match(serviceWorker, /favorite-editor-ui\.css\?v=1\.10\.1/);
  assert.match(serviceWorker, /archive-viewer-layout\.js\?v=1\.10\.1/);
  assert.match(serviceWorker, /archive-viewer-layout-core\.mjs/);
  assert.match(serviceWorker, /archive-viewer-layout\.css\?v=1\.10\.1/);
  assert.match(serviceWorker, /image-navigation\.js\?v=1\.10\.1/);
  assert.match(serviceWorker, /image-metadata\.mjs/);
  assert.match(serviceWorker, /image-viewer-fit\.js\?v=1\.10\.1/);
  assert.match(serviceWorker, /prompt-organization-backup\.js\?v=1\.10\.1/);
  assert.match(serviceWorker, /prompt-organization-backup\.css\?v=1\.10\.1/);
