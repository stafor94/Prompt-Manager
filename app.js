...
async function getAllPrompts() {
  return getAllPromptSummaries();
}

async function getPrompt(id) {
  return getPromptRecord(id);
}

async function putPrompt(prompt) {
  return putPromptRecord(prompt);
}

async function deletePrompt(id) {
  return deletePromptRecord(id);
}
...
async function ensurePromptVersions(prompts) {
  ...
  for (const index of normalized.changedIndexes) {
    const summary = normalized.prompts[index];
    const prompt = await getPromptRecord(summary.id);
    if (prompt) await putPromptRecord({ ...prompt, version: summary.version });
  }
...
function renderPromptList() {
  ...
  elements.promptList.innerHTML = prompts.map((prompt) => {
    const imageCount = getStoredImageCount(prompt);
    return `
      <button class="prompt-card" type="button" data-prompt-id="${prompt.id}">
        ...
        <div class="prompt-meta">
          ...
          ${imageCount > 0 ? `<span>이미지 ${imageCount}장</span>` : ""}
...
async function submitPrompt(event) {
  ...
  const prompt = {
    ...
    ...(Array.isArray(previous?.tags) ? { tags: previous.tags } : {}),
    version,
...
function requestCloseImageViewer() {
  ...
  if (state.imageViewerHistoryActive) history.back();
  else closeImageViewerDirectly();
}
...
function bindViewerGestures() {
  ...
  stage.addEventListener("dblclick", () => {
    if (state.viewerTransform.scale > 1) resetViewerTransform();
    else {
